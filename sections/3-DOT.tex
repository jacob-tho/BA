...
\begin{definition}[Absolutely Continuous Curves]
	...
	$$
	holds for all $t<s$. We write $\gamma\in \mathrm{AC}(X)$.
\end{definition}
...
\end{lemma}
\begin{proof}
	Let $\gamma:[0,1]\to X$ be absolutely continuous with $L(\gamma) = \int_0^1|\gamma'|(t)dt<\infty$. The arc-lenght is defined by $s(t) := \int_0^t |\gamma'|(t)dt$ with $s(0) = 0$ and $s(1) = L(\gamma)$. Reparametrizing by arc-length is defined by 
...
\begin{theorem}
	...
	\end{itemize}
\end{theorem}
The idea of the proof is to construct a sequence of $\gamma_t$ and a corresponding sequence for $v_t$ such that the continuity equation is solved, $\gamma_t^k$ is absolutely continuous for all $k$ and the limit admits the desired inequality.
\begin{proof}
By Lemma ?? we assume $\mu$ to be Lipschitz. We define the sequence $\mu_t^k:= \mu_t * \eta_k$, where $\nu_k$ is a Dirac sequence supported on $B\left(0,\frac{1}{k}\right)$. This is a necessary step, since for example $\delta_x \in \mathscr{P}_2(X)$ is not absolutely continuous. Since
	$$
